<!-- ---
type: lecture
date: 2025-09-16T17:50:00
title: "Lecture 14: Bellman Equation for Action-Value and Backup Diagram"
tldr: "Model-based IV"
stat: lec
# for lectures stat: lec
description: We extend the derivation of Bellman equation to the action-value function. We see that the Bellman equations describe a simple graphic diagram which can relate the value of each state or action-state to other values in the environment. 
videoID: TroWF59EdAI 
hide_from_announcments: false
---
**Lecture Notes:**
- [Chapter 2 - Section 1]({{ site.baseurl }}/assets/Notes/CH2/CH2_Sec1.pdf) 

**Further Reads:**
* [Bellman](http://incompleteideas.net/book/the-book-2nd.html): Chapter 3 - Section 3.6 [[SB]](http://incompleteideas.net/book/the-book-2nd.html) -->

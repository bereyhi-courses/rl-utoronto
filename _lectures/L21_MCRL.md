---
type: lecture
date: 2025-09-23T18:10:00
title: "Lecture 21: Model-free Policy Evaluation via Monte-Carlo"
tldr: "Monte-Carlo - Part I"
stat: lec
# for lectures stat: lec
description: We start with model-free approaches for RL. As the first stop, we look into Monte-Carlo approach and see how we could use it to estimate the value function.
videoID: CN69-xSnMGM 
hide_from_announcments: false
---
**Lecture Notes:**
- [Chapter 3 - Section 1]({{ site.baseurl }}/assets/Notes/CH2/CH3_Sec1.pdf) 

**Further Reads:**
* [Monte-Carlo](http://incompleteideas.net/book/the-book-2nd.html): Chapter 5 - Sections 5.1 and 5.2 of [[SB]](http://incompleteideas.net/book/the-book-2nd.html)